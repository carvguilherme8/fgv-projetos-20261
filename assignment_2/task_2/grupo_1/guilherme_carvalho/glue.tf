# ---------------------------------------------------------------------------
# Glue JDBC Connection to RDS MySQL
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Upload incremental ETL script to S3
# ---------------------------------------------------------------------------
...
resource "aws_glue_job" "etl" {
  ...
  default_arguments = {
    "--job-language"          = "python"
    "--S3_OUTPUT_PATH"        = "s3://${aws_s3_bucket.data_lake.id}/analytics"
    "--JDBC_CONNECTION_URL"   = "jdbc:mysql://${local.rds_endpoint}/${var.rds_database}"
    ...
    # pymysql is needed by the ETL script to update the watermark table
